...
class _BookingSlotsComponentState extends State<BookingSlotsComponent> {
  ...
  Widget build(BuildContext context) {
    List<String> temp = slotsList.validate().isNotEmpty
        ? slotsList.validate().firstWhere((element) => element.day!.toLowerCase() == DateFormat('E', 'en_US').format(selectedHorizontalDate).toLowerCase(), orElse: () => SlotData(slot: [], day: '')).slot.validate()
        : [];
    return Container(
      child: Column(
        children: [
